...
APP_CLIENT_NAME="InsuranceSystem"
...
aws cognito-idp create-user-pool-domain \
    --domain insurance-system-domain \
    --user-pool-id us-east-1_k032kDiSf

change "Branding version" to "Managed login"
go to App Clients (App client information) and then InsuranceSystem and press Edit
Check:
X Choice-based sign-in: ALLOW_USER_AUTH
X Sign in with secure remote password (SRP): ALLOW_USER_SRP_AUTH
X Get new user tokens from existing authenticated sessions: ALLOW_REFRESH_TOKEN_AUTH
