...
function displayClaimsList(data) {
  ...
  if (!claims || claims.length === 0) {
    html +=
      '<div class="alert alert-info"><i class="fas fa-info-circle"></i> No claims found for this policy</div>';
  } else {
...
function showAddPolicy() {
  ...
  $("#policyNumber")
    .attr("placeholder", `POL-${new Date().getFullYear()}-`)
    .prop("disabled", false);
  $("#searchSection").hide();
...
async function savePolicy() {
  const policyData = {
    ...
    insured: {
      ...
      email: $("#insuredEmail").val().trim(),
      phone: $("#insuredPhone").val().trim(),
      idNumber: $("#insuredId").val().trim(),
    },
    vehicle: {
...
async function submitClaim(e) {
  const claimData = {
    policyNumber: $("#claimPolicyNumber").val().trim(),
    claimNumber: $("#claimNumber").val().trim(),
    claimDate: $("#claimDate").val(),
    description: $("#claimDescription").val().trim(),
  };
  if (
...
async function confirmDelete() {
  await apiCallAsync(
    "DELETE",
    `policies/${currentPolicyToDelete}`,
    null,
    function () {
...
function approveClaim(btn, policyNumber, claimNumber, assessmentValue) {
  ...
  approveClaimModalHeader.html(
    '<i class="fa-solid fa-thumbs-up"></i> Claim confirmation'
  );
  approveClaimModalText.html(
    `Are you sure you want to approve claim <strong>${claimNumber}</strong> for <strong>${CURRENCY_SIGN} ${assessmentValue.toLocaleString()}</strong> ?`
  );
  approveClaimModalBtn
    .off("click")
    .on("click", function () {
      confirmClaimApprove(true);
    })
    .text("Approve Claim");
...
function rejectClaim(btn, policyNumber, claimNumber, assessmentValue) {
  ...
  const approveClaimModalBtn = $("#approveClaimModalBtn").empty();
  approveClaimModalHeader.html(
    '<i class="fa-solid fa-thumbs-down"></i> Claim rejection'
  );
  approveClaimModalText.html(
    `Are you sure you want to reject claim <strong>${claimNumber}</strong> for <strong>${CURRENCY_SIGN} ${assessmentValue.toLocaleString()}</strong> ?`
  );
  approveClaimModalBtn
    .off("click")
    .on("click", function () {
      confirmClaimApprove(false);
    })
    .text("Reject Claim");
  new bootstrap.Modal($("#approveClaimModal")).show();
}
